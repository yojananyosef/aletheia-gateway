...
<aside class="neo-sidebar {isCollapsed ? 'is-collapsed' : ''} {isOpen ? 'is-open' : ''}">
  <div class="neo-sidebar-header">
    <button
      type="button"
      class="neo-brand-btn {isCollapsed ? 'is-collapsed-btn' : ''}"
  ...
  <div class="neo-sidebar-content">
    <div class="neo-sidebar-section">
      ...
      {#each mainItems as item}
        ...
        <button
          type="button"
          class="neo-nav-button {activeView === item.view && item.title !== 'Buscar pasajes' ? 'is-active' : ''} {isCollapsed ? 'is-collapsed-btn' : ''}"
          title={item.title}
    ...
    <div class="neo-sidebar-section">
      ...
      {#each exploreItems as item}
        ...
        <button
          type="button"
          class="neo-nav-button {isCollapsed ? 'is-collapsed-btn' : ''}"
          title={item.title}
        >
          <Icon size={20} class="shrink-0" />
          {#if !isCollapsed}
